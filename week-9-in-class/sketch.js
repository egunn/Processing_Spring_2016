...
function draw() {
    ...
    if(mouseListener ==true) {
        ...
        for(var i=0; i<investorsToDisplay.length;i++){
            noStroke();
            fill(190,100,90,.7);
            ellipse(investorsToDisplay[i].x,investorsToDisplay[i].y,15,15); 
             
            //draw line to investors
            stroke(100,100,40,.7)
            line(investorsToDisplay[i].x,investorsToDisplay[i].y,companyToDisplay.pos.x,
                 companyToDisplay.pos.y);
